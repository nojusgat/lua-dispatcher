...
local GroupsEndpoint = {}
GroupsEndpoint.__index = GroupsEndpoint

setmetatable(GroupsEndpoint, { __index = BaseEndpoint })

function GroupsEndpoint:init()
    self:enable_auth("post")
end

function GroupsEndpoint:get()
    local where = {}
    ...
    local total_count = self.model["Groups"]
        :get()
        :where(where)
        :count()
    ...
    local results = self.model["Groups"]
        :get()
        :where(where)
        :limit(limit)
        :offset(offset)
...
function GroupsEndpoint:post()
    ...
    local status, results = pcall(function()
        local group = self.model["Groups"](data)
        group:save()
        return group
    end)
