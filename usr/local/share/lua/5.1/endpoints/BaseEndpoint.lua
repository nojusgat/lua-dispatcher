...
setmetatable(BaseEndpoint, { __index = Endpoint })
...
function BaseEndpoint:query_to_string_table(query, allowed)
    if not allowed then allowed = {} end
    local strings = {}
    local counter = 0
    ...
    if counter == 0 then
        self.send({ error = query .. " is not a valid query" }, 400)
        os.exit()
    end
    strings["__n"] = counter
    return strings
...
function BaseEndpoint:pagination(total_count, limit)
    ...
    if page > total_pages then
        self.send({ error = "Page does not exist" }, 400)
        os.exit()
    end
...
function BaseEndpoint:upload_image(image)
    if type(image) ~= "table" or not image.data or not image.filename or not image.mime then
        self.send({ error = "Avatar should be a file" }, 400)
        os.exit()
    end
    ...
    if not allowed_mime_types[file_type] then
        self.send({ error = "File format not accepted" }, 400)
        os.exit()
    end
    ...
    if not file then
        self.send({ error = "Failed to upload file" }, 500)
        os.exit()
    end
...
function BaseEndpoint:permission(permission)
    if permission == "system_admin" then
        if self.auth_data.user.system_admin ~= true then
            self.send({ error = "Permission denied" }, 403)
            os.exit()
        end
        return
    end
...
function BaseEndpoint:authorize()
    ...
    local user = model["Users"]
        :get()
        :select({ "id", "username", "name", "email", "avatar", "system_admin", "exp" })
        :where({ id = id })
        :find_one()
    ...
    local permissions = model["UserPermissions"]
        :get()
